...
CREATE OR REPLACE FUNCTION data_contst_func_9() RETURNS trigger AS $BODY$
	...
	BEGIN
		SELECT EduLevel,jobType,email INTO t_EduLevel,t_JobType,t_sender_email FROM public."Advertisment" INNER JOIN public."Job_Offer"  ON public."Advertisment".advertismentID = NEW.advertismentID;
		FOR t_receiver_email IN (SELECT email FROM public."Advertisment" INNER JOIN public."Job_Offer"  ON public."Advertisment".advertismentID = public."Job_Offer".advertismentID  WHERE EduLevel = t_EduLevel AND jobType = t_JobType) LOOP
...
CREATE OR REPLACE FUNCTION data_contst_func_10() RETURNS trigger AS $BODY$
	...
	DECLARE t_JobType varchar(30);
	BEGIN
		SELECT email,jobType INTO t_sender_email,t_JobType FROM public."Advertisment" INNER JOIN public."Job_Seek"  ON public."Advertisment".advertismentID = NEW.advertismentID;
		FOR t_receiver_email IN SELECT email FROM public."Advertisment" INNER JOIN public."Job_Offer"  ON public."Advertisment".advertismentID = public."Job_Offer".advertismentID WHERE jobType = t_JobType LOOP
			INSERT INTO public."Msg"(msgID,dateSent,theSubject,theText,sender_email,receiver_email)
...
begin
	...
	IN (
	SELECT public."Education".email FROM public."Education" 
	WHERE  public."Education".school=t_school 
	AND public."Education".email NOT IN (SELECT public."myview".email2 FROM public."myview" WHERE public."myview".email1=$1) 
	AND ((public."Education".toYear <= t_toYear and public."Education".toYear >= t_fromYear) 
	OR (public."Education".fromYear >= t_fromYear and public."Education".toYear <= t_toYear))
	); 
...
begin 
	...
	SELECT * FROM public."Member" WHERE public."Member".email 
	IN ( 
	SELECT public."Article_Comment".email FROM public."Article_Comment" 
	GROUP BY public."Article_Comment".email 
	HAVING COUNT(DISTINCT articleID) = (SELECT COUNT(public."Article".email) FROM public."Article" WHERE public."Article".email=$1) 
	INTERSECT
	SELECT public."Article_Comment".email FROM public."Article_Comment" WHERE public."Article_Comment".articleID IN (SELECT public."Article".articleID FROM public."Article" WHERE public."Article".email=$1)
	);
end;
$BODY$
LANGUAGE 'plpgsql' ;
...
--ARTICLE/ARTICLE COMMENT
	INSERT INTO public."Article" VALUES(1,'The Alchemist- Review',6,'The Alchemist By Paulo Coelho',NOW(),'test@domain.gr'),
					   (2,'DAVINCI CODE -Review',6,'DaVincis Code By Dan Brown',NOW(),'test1@domain.gr'),
					   (3,'Future Economy',7,'Specs of the worlds future economy',NOW(),'test4@domain.gr');
	INSERT INTO public."Article_Comment" VALUES(1,'MyComment For your Review',NOW(),1,'test2@domain.gr'),
						   (2,'MyComment For your Review',NOW(),1,'test1@domain.gr'),
						   (3,'MyComment For your Review',NOW(),1,'test5@domain.gr');
...
--MESSAGE
	INSERT INTO public."Msg" VALUES((SELECT COUNT(*) FROM public."Msg")+1,'2016-04-04','Invite For Lecture','Dear Friend,I want to invite you to my lecture','test@domain.gr','test1@domain.gr');
...
-- 1
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Limit messages for specific user per day 
CREATE OR REPLACE FUNCTION limit_messages() RETURNS trigger AS $BODY$
	DECLARE cnt INTEGER;
	DECLARE t_email varchar(30);
	BEGIN
		t_email = NEW.receiver_email;
		SELECT COUNT(*) INTO cnt FROM public."Msg" WHERE receiver_email=t_email AND dateSent = NEW.dateSent;
		IF cnt + 1 > 10 THEN 
			RAISE EXCEPTION 'User with email : % has reached the limit of inbox messages for today',t_email;
		END IF;
		RETURN NEW;
	END;
$BODY$ LANGUAGE 'plpgsql' ;
CREATE TRIGGER limit_messages BEFORE INSERT ON public."Msg" FOR EACH ROW EXECUTE PROCEDURE limit_messages();

--Limit invitation for specific user per day
CREATE OR REPLACE FUNCTION limit_invitations() RETURNS trigger AS $BODY$
	DECLARE cnt INTEGER;
	DECLARE t_email varchar(30);
	BEGIN
		t_email = NEW.receiver_email;
		SELECT COUNT(*) INTO cnt FROM public."Invitation" WHERE receiver_email=t_email AND dateSent = NEW.dateSent;
		IF cnt + 1 > 5 THEN 
			RAISE EXCEPTION 'User with email : % has reached the limit of friendship invitations for today',t_email;
		END IF;
		RETURN NEW;
	END;
$BODY$ LANGUAGE 'plpgsql' ;
CREATE TRIGGER limit_invitations BEFORE INSERT ON public."Invitation" FOR EACH ROW EXECUTE PROCEDURE limit_invitations();

--Limit recommendation_requests for specific user per day
CREATE OR REPLACE FUNCTION limit_rec_requests() RETURNS trigger AS $BODY$
	DECLARE cnt INTEGER;
	DECLARE t_email varchar(30);
	BEGIN
		t_email = NEW.receiver_email;
		SELECT COUNT(*) INTO cnt FROM public."Recommendation_Request" WHERE receiver_email=t_email AND dateSent = NEW.dateSent;
		IF cnt + 1 > 5 THEN 
			RAISE EXCEPTION 'User with email : % has reached the limit of recommendation requests for today',t_email;
		END IF;
		RETURN NEW;
	END;
$BODY$ LANGUAGE 'plpgsql' ;
CREATE TRIGGER limit_rec_requests BEFORE INSERT ON public."Recommendation_Request" FOR EACH ROW EXECUTE PROCEDURE limit_rec_requests();



CREATE OR REPLACE FUNCTION public.bulkactions(
    date,
    date)
  RETURNS void AS
$BODY$
	...
	DECLARE t_articleID smallint;
	DECLARE s_email varchar(30); 
	DECLARE r_email varchar(30); -- article author
	DECLARE cnt integer;
	DECLARE t_cnt integer;
	DECLARE f_text varchar;
	DECLARE t_text varchar;
	BEGIN 
	t_cnt := 0;
	SELECT ( (SELECT COUNT(*)::INTEGER FROM public."Msg" WHERE dateSent = NOW()::DATE) + (SELECT COUNT(*)::INTEGER FROM public."Recommendation_Request" WHERE dateSent = NOW()::DATE) ) INTO cnt;
	OPEN curs FOR SELECT email,articleID FROM public."Article_Comment" WHERE datePosted >=$1 AND datePosted<=$2;
		LOOP
		FETCH curs INTO s_email,t_articleID;
			EXIT WHEN NOT FOUND;
		BEGIN 
			t_text := '';
			f_text := '';
			SELECT email INTO r_email FROM public."Article" WHERE articleID =t_articleID;
			IF (r_email <> s_email) THEN
				--1
				IF (SELECT COUNT(*) FROM public."myview" WHERE email1=s_email AND email2=r_email AND lvl=1)>0 THEN 
					-- a 
					IF (SELECT COUNT(*) FROM public."Experience" a INNER JOIN public."Experience" b 
					ON a.company = b.company
					WHERE a.email = s_email AND b.email=r_email 
					AND ((a.toYear<=b.toYear AND a.toYear>=b.fromYear) OR (a.fromYear>=b.fromYear AND a.toYear<=b.toYear)))>0 
					THEN
						INSERT INTO public."Recommendation_Request"(requestID,dateSent,theStatus,for_email,sender_email,receiver_email)
						VALUES ((SELECT COUNT(*) FROM public."Recommendation_Request")+1,NOW(),'PENDING',s_email,r_email,s_email);  
						t_cnt := t_cnt + 1; 
					END IF;
					--b 
					f_text := 'Common Interests:';
					OPEN curs1 FOR (SELECT Aname FROM public."Category" WHERE categoryID IN
					(SELECT a.interestID FROM public."Member_Interests" a INNER JOIN public."Member_Interests" b 
					ON a.interestID = b.interestID 
					WHERE a.memberID = s_email AND b.memberID=r_email)); 
					LOOP
						FETCH curs1 INTO t_text;
							EXIT WHEN NOT FOUND;
						f_text := f_text || '  ' || t_text;
					END LOOP; 
					INSERT INTO public."Endorses"(email,recommended_email,skills,datePosted)
					VALUES(r_email,s_email,f_text,NOW());
					--c
					INSERT INTO public."Msg"(msgID,dateSent,theSubject,theText,sender_email,receiver_email)
					VALUES ((SELECT COUNT(*) FROM public."Msg")+1,NOW(),'THANK YOU t1','Thank you for your comment',r_email,s_email);
					t_cnt := t_cnt + 1;
				--2
				ELSIF (SELECT COUNT(*) FROM public."myview" WHERE email1=s_email AND email2=r_email AND lvl=2)>0 THEN 
					--a
					SELECT title INTO t_text FROM public."Experience" 
					WHERE email=r_email AND toYear=(SELECT MAX(toYear) FROM public."Experience" WHERE email=r_email);
					f_text := t_text;
					SELECT summary INTO t_text FROM public."Summary" WHERE email=r_email ;
					f_text := f_text || ' ' || t_text;
					INSERT INTO public."Msg"(msgID,dateSent,theSubject,theText,sender_email,receiver_email)
					VALUES((SELECT COUNT(*) FROM public."Msg")+1,NOW(),'Message For Connection Type 2',f_text,r_email,s_email);
					t_cnt := t_cnt + 1;
					INSERT INTO public."Invitation"(invitationID,dateSent,theStatus,sender_email,receiver_email)
					VALUES ((SELECT COUNT(*) FROM public."Invitation")+1,NOW(),'PENDING',r_email,s_email); 
					--b
					INSERT INTO public."Msg"(msgID,dateSent,theSubject,theText,sender_email,receiver_email)
					VALUES ((SELECT COUNT(*) FROM public."Msg")+1,NOW(),'THANK YOU t2','Thank you for your comment',r_email,s_email);
					t_cnt := t_cnt + 1;
				--3
				ELSIF (SELECT COUNT(*) FROM public."myview" WHERE email1=s_email AND email2=r_email AND lvl>=3)>0 THEN 
					--a
					SELECT COUNT(*)::varchar INTO t_text FROM public."Article" 
					WHERE email=s_email AND categoryID=(SELECT categoryID FROM public."Article" WHERE articleID=t_articleID);
					f_text := 'Number of posted Articles in the same category: ' || t_text;
					SELECT ' ,from Member with email:'||email||' ,Name:'||firstName||' '||secondName||' ,from country: '||country
					INTO t_text
					FROM public."Member" WHERE email=s_email;
					f_text := f_text || t_text;
					INSERT INTO public."Msg"(msgID,dateSent,theSubject,theText,sender_email,receiver_email)
					VALUES((SELECT COUNT(*) FROM public."Msg")+1,NOW(),'Message For Connection Type 3',f_text,s_email,r_email);
					t_cnt := t_cnt + 1;
					--b
					INSERT INTO public."Msg"(msgID,dateSent,theSubject,theText,sender_email,receiver_email)
					VALUES ((SELECT COUNT(*) FROM public."Msg")+1,NOW(),'THANK YOU t3','Thank you for your comment',r_email,s_email);
					t_cnt := t_cnt + 1;
				END IF;
			END IF;
			IF (t_cnt >= cnt*2 AND cnt <> 0) THEN 
				ROLLBACK;
			END IF;
			EXCEPTION WHEN others THEN
				RAISE EXCEPTION 'The transaction cannot be completed';
		END;
		END LOOP;
	END;
$BODY$
  ...
  COST 100;
ALTER FUNCTION public.bulkactions(date, date)
  OWNER TO postgres;

CREATE OR REPLACE FUNCTION public.CreateJunkData()
  RETURNS void AS
$BODY$
	DECLARE t_check boolean;
	DECLARE t_email varchar(60);
	DECLARE t_counter integer;
	BEGIN
		t_check := true;
		t_counter :=0;
		WHILE t_counter<501 LOOP
			t_email := 'junk' || t_counter::varchar || '@domain.gr';
			INSERT INTO public."Member" VALUES(t_email,t_counter::varchar,t_counter::varchar,t_counter::varchar,'2018-08-07','2015-05-05','GREECE',11145,'MALE','2016-04-06');
			INSERT INTO public."Education" VALUES((SELECT COUNT(*) FROM public."Education")+1,'GREECE','SCHOOL','HIGH',1,1995,2005,t_email);
			INSERT INTO public."Article" VALUES((SELECT COUNT(*) FROM public."Article")+1,'THE TITLE',7,'THE TEXT',NOW(),t_email);
			INSERT INTO public."Article_Comment" VALUES((SELECT COUNT(*) FROM public."Article_Comment")+1,'THE COMMENT',NOW(),(SELECT COUNT(*) FROM public."Article"),t_email);
			IF t_check = TRUE THEN 
			INSERT INTO public."Advertisment" VALUES((SELECT COUNT(*) FROM public."Advertisment")+1,'800',t_counter::varchar,'MEGA',t_counter::varchar,'NONE','ADVERTISMENT1','LOCATION',11145,'GREECE','2016-03-04',t_email); 
			INSERT INTO public."Job_Offer" VALUES((SELECT COUNT(*) FROM public."Advertisment"),'DESCRIPTION',20,45,'ANT1','ANT1.gr');
			t_check := FALSE;
			ELSE
			INSERT INTO public."Advertisment" VALUES((SELECT COUNT(*) FROM public."Advertisment")+1,'1200',t_counter::varchar,'MEGA1',t_counter::varchar,'NONE','ADVERTISMENT11','LOCATION1',11145,'GREECE','2016-03-04',t_email);
			INSERT INTO public."Job_Seek" VALUES((SELECT COUNT(*) FROM public."Advertisment"),'DESCRIPTION1');
			t_check := TRUE;
			END IF;
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION public.CreateJunkData()
  OWNER TO postgres;

SELECT public."Initialize"();
--SELECT public."createjunkdata"();

CREATE INDEX education_email_idx ON public."Education" USING BTREE(email);
CREATE INDEX education_school_idx ON public."Education" USING HASH(school);
CREATE INDEX education_fromYear_idx ON public."Education" USING BTREE(fromYear);
CREATE INDEX education_toYear_idx ON public."Education" USING BTREE(toYear);
CLUSTER public."Education" USING education_email_idx;

CREATE INDEX article_email_idx ON public."Article" USING HASH(email);
CREATE INDEX article_articleID_idx ON public."Article" USING BTREE(articleID);
CLUSTER public."Article" USING article_articleID_idx;

CREATE INDEX article_comment_articleID_idx ON public."Article_Comment" USING BTREE(articleID);
CREATE INDEX article_comment_email_idx ON public."Article_Comment" USING HASH(email);
CLUSTER public."Article_Comment" USING article_comment_articleID_idx;

CREATE INDEX advertisment_jobType_idx ON public."Advertisment" USING BTREE(jobType);
CREATE INDEX advertisment_industry_idx ON public."Advertisment" USING HASH(industry);
CREATE INDEX advertisment_salary_idx ON public."Advertisment" USING BTREE(salary);
CREATE INDEX advertisment_country_idx ON public."Advertisment" USING HASH(country);
CLUSTER public."Advertisment" USING advertisment_jobType_idx;
